import os, json, time, logging, requests, signal
...
# ---------------------- КОНФИГ ----------------------
TG_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")  # токен от @BotFather
CHAT_ID = os.getenv("LOG_CHAT_ID")  # id канала (-100...) или @username
PRODUCT = os.getenv("PRODUCT_TYPE", "USDT-FUTURES")
TF = os.getenv("TF", "15m")
SLEEP_SEC = int(os.getenv("SCAN_INTERVAL_SEC", "60"))
LEVERAGE = float(os.getenv("LEVERAGE", "20"))

KC_N = int(os.getenv("KC_N", "20"))
KC_M = float(os.getenv("KC_M", "2.0"))
...
def signal_from_prev_bar(meta: Dict[str, Any]) -> str:
    ...
    if long_break and ema_long_ok and rsi_long_ok:
        return "LONG"
    if short_break and ema_short_ok and rsi_short_ok:
        return "SHORT"
    return ""
...
def format_open_msg(symbol: str, side: str, entry_low: float, entry_high: float, tps: List[float], sl: float):
    ...
    lines = [
        f"📌 #{base}/USDT",
        f"Leverage: Cross {int(LEVERAGE)}X",
        f"⚠️ Signal: {'long' if side == 'LONG' else 'short'}",
        "",
        "☑️ Entry Zone:",
    ...
    lines += [
        "",
        f"⛔️ Stop-Loss: {fmt_price(sl)}",
        "",
        "Риск-менеджмент:",
        "Не более 2-3% депозита на одну сделку"
    ]
    return "\n".join(lines)
    ...
    while True:
        try:
            for symbol in state["allowed_symbols"]:
                ...
                if len(df) < max(EMA_SLOW, KC_N) + 5:
                    continue
                last_ts = int(df["ts"].iloc[-1])
                if state["symbols_seen"].get(symbol) == last_ts:
                    continue
